"""
dashboard/server.py — JUDO Local HTTP Dashboard
...
def _ensure_network_access(port: int) -> None:
    ...
    if sys.platform == "win32":
        ...
        port_rule = f"JUDO Dashboard Port {port}"
        prog_rule  = "JUDO Dashboard Python"
        ...
        bat_body = "\r\n".join(bat_lines) + "\r\n"
        fd, bat_path = tempfile.mkstemp(suffix=".bat", prefix="judo_fw_")
        try:
        ...
        try:
            ...
            if int(ret) > 32:
                ...
                print("[Dashboard] Phone connections may fail until JUDO is run as Administrator.")
...
class DashboardServer:
    ...
    def _ssl_enabled() -> bool:
        ...
        return (certs / "judo.key").exists() and (certs / "judo.crt").exists()
...
<p>Press <strong style="color:#dde3ed">Remote Control</strong> in JUDO to get a new QR code.</p>
</div></body></html>""")
...
<body>
<script>
  sessionStorage.setItem('judo_token','{tok}');
  sessionStorage.setItem('judo_key','{key}');
  localStorage.setItem('judo_device_token','{dev_tok}');
  setTimeout(function(){{location.replace('/')}},400);
</script>
<p>Connecting to JUDO…</p>
</body></html>""")
    ...
    async def _serve_alias(self) -> None:
        ...
        ssl_key  = BASE_DIR / "config" / "certs" / "judo.key"
        ssl_cert = BASE_DIR / "config" / "certs" / "judo.crt"
    ...
    async def serve(self) -> None:
        ...
        print("[Dashboard] Press 'Remote Control' in JUDO UI to get the QR code.")
        await uvicorn.Server(cfg).serve()
